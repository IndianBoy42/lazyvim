...
M.repeatable = function(ch, desc, fwdbwd, _opts)
  ...
  local cfg = {
    on_enter = function()
      require("keymaps.nN_repeat").register(fwdbwd)
    end,
    on_key = function()
      vim.wait(50)
    end,
...
M.move_by = function(prefix, suffixes, actions, desc, o)
  ...
  local hydras = {}
  if #prefix == 0 or prefix == false then
    prefix = nil
  end
  local heads = {}
...
M.jump_mappings = function()
  local ai = require("mini.ai")
  local function mapall(id, desc, sym)
    desc = desc or legend[id] or ""
    local mc = ai.move_cursor
    local st = ai.select_textobject
    -- TODO: this should use mini.bracketed
    local w = function()
      mc("left", "i", id, { search_method = "next" })
    end
    local e = function()
      mc("right", "i", id, { search_method = "cover_or_next" })
    end
    local b = function()
      mc("left", "i", id, { search_method = "cover_or_prev" })
    end
    local ge = function()
      mc("right", "i", id, { search_method = "prev" })
    end
    local W = function()
      mc("left", "a", id, { search_method = "next" })
    end
    local E = function()
      mc("right", "a", id, { search_method = "cover_or_next" })
    end
    local B = function()
      mc("left", "a", id, { search_method = "cover_or_prev" })
    end
    local gE = function()
      mc("right", "a", id, { search_method = "prev" })
    end
    local vi = function()
      st("i", id, { search_method = "cover" })
    end
    local va = function()
      st("a", id, { search_method = "cover" })
    end
    local vin = function()
      st("i", id, { search_method = "next" })
    end
    local van = function()
      st("a", id, { search_method = "next" })
    end
    local vip = function()
      st("i", id, { search_method = "prev" })
    end
    local vap = function()
      st("a", id, { search_method = "prev" })
    end
    M.repeatable(id, desc, { w, b, e, ge }, {})
    if sym then
      local hydra = M.move_by(sym, M.move_by_suffixes, { w, b, e, ge, W, B, E, gE, vi, va, vin, vip, van, vap }, desc)
